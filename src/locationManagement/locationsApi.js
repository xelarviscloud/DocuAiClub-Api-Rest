...
locationRouter.get("/v2/location/get", async (req, res) => {
    try {
        ...
        const promises = locationData.map(async (locationDetail) => {
            const organizationData = await organizationDetails(locationDetail.organizationid);
            const parentOrganization = organizationData ? organizationData.name : null;
            return {
                data: locationDetail,
                parentOrganization,
            };
 ...
 */
locationRouter.get(
    "/v2/location/get/:locationid",
    async (req, res) => {
        try {
            const locationid = req.params.locationid;

            // Query the database for location data based on location ID, pagination, and sorting
            const locationData = await Location.find({ locationid, isDeleted: { $ne: true } });
